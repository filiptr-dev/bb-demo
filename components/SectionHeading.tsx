  ...
  return (
    <div className={`relative mb-10 ${center ? "text-center" : ""}`}>
      ...
      <Reveal y={-15} className={`relative flex items-center gap-3 mb-4 ${center ? "justify-center" : ""}`}>
        <div className={`w-10 h-[2px] bg-gradient-to-r ${center ? "from-transparent to-brand-1" : "from-brand-1 to-brand-2"}`} />
        <span className="text-brand-2 text-xs tracking-[0.3em] uppercase font-semibold">{eyebrow}</span>
        {center && <div className="w-10 h-[2px] bg-gradient-to-r from-brand-1 to-transparent" />}
      </Reveal>
      <Reveal delay={0.15}>
