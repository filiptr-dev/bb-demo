  ...
  return (
    <header className={`fixed top-0 inset-x-0 z-50 transition-all duration-300 ${scrolled ? "bg-background/80 backdrop-blur-xl border-b border-border" : "bg-transparent"}`}>
      <div className="container mx-auto px-6 lg:px-10 h-16 flex items-center gap-8">
        ...
        <Button nativeButton={false} render={<Link href="/catalog" />} className="ml-auto md:ml-0 hidden sm:inline-flex rounded-full bg-brand-gradient text-white text-xs font-semibold uppercase tracking-wide h-9 px-5 hover:shadow-[0_0_30px_rgba(255,90,50,0.45)] transition-shadow">Е-каталог</Button>
